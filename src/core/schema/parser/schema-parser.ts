...
export class SchemaParser {
  ...
  private autoQuoteReservedKeywords(sql: string): string {
    ...
    for (const keyword of keywords) {
      ...
      const pattern = new RegExp(`(?<![CREATE|ALTER]\\s{1,20})(?<=\\s|,|\\()\\b${keyword}\\b(?=\\s+(INTEGER|INT|INT2|INT4|INT8|SMALLINT|BIGINT|VARCHAR|TEXT|BOOLEAN|BOOL|TIMESTAMP|DATE|TIME|NUMERIC|DECIMAL|REAL|DOUBLE|SERIAL|BIGSERIAL|UUID|JSONB|JSON|MONEY|BIT|VARBIT))`, 'gi');

...
describe("Edge case: serial/bigserial columns", function () {
  ...
  test("rejects clauses that conflict with serial before preceding mutations", async function () {
    const baseline = `
      CREATE TABLE serial_clause_guard (
        id INTEGER
      );
    `;
    ...
    const conflictingDefinitions = [
      "SERIAL NULL",
      "SERIAL4 DEFAULT 42",
      "SERIAL8 GENERATED BY DEFAULT AS IDENTITY",
      "SERIAL2 GENERATED ALWAYS AS (1) STORED",
    ];
    for (const definition of conflictingDefinitions) {
      const invalidDesired = `
        CREATE TABLE serial_clause_guard (
          id INTEGER,
          pending TEXT
        );
        CREATE TABLE invalid_serial_clause (
          id ${definition}
        );
      `;
      ...
      await expect(
        ...
        message: expect.stringMatching(
          /serial column.*conflict.*implicit.*NOT NULL.*default/i
        ),
      });
      ...
      const invalidTable = await client.query(
        "SELECT to_regclass('public.invalid_serial_clause') AS relation"
      );
    ...
    const explicitNotNull = `
      ${baseline}
      CREATE TABLE explicit_serial_not_null (
        id SERIAL NOT NULL
      );
    `;
    await schemaService.apply(explicitNotNull, ["public"], true);
    expect(
      (await schemaService.plan(explicitNotNull, ["public"])).hasChanges
    ).toBe(false);
  });

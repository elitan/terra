...
describe("Edge case: array column types and modifications", () => {
  ...
  async function expectArrayColumnTypes(expectedTypes: string[]) {
    const columns = await getTableColumnDetails(client, "logs");
  ...
  test("v1: create and verify idempotency", async () => {
    ...
    await expectArrayColumnTypes(["character varying(255)[]"]);

...
describe("PostgreSQL generated-column function dependencies", function () {
  ...
  test("rejects generated columns with defaults before mutation", async function () {
    const service = createTestSchemaService();
    const desired = `
      CREATE TABLE public.unrelated_generated_default_records (
        id integer PRIMARY KEY
      );

      CREATE TABLE public.generated_default_records (
        source text NOT NULL,
        normalized text GENERATED ALWAYS AS (lower(source)) STORED DEFAULT 'fallback'
      );
    `;

    await expect(service.plan(desired, ["public"])).rejects.toMatchObject({
      code: "VALIDATION_ERROR",
      message: expect.stringContaining("cannot declare a default"),
    });
    expect(
      (await client.query(`
        SELECT
          to_regclass('public.unrelated_generated_default_records') AS unrelated,
          to_regclass('public.generated_default_records') AS invalid
      `)).rows
    ).toEqual([{ unrelated: null, invalid: null }]);
  });

  test("rejects generated columns with identities before mutation", async function () {
    const service = createTestSchemaService();
    const desired = `
      CREATE TABLE public.unrelated_generated_identity_records (
        id integer PRIMARY KEY
      );

      CREATE TABLE public.generated_identity_records (
        source text NOT NULL,
        normalized text GENERATED ALWAYS AS (lower(source)) STORED GENERATED BY DEFAULT AS IDENTITY
      );
    `;

    await expect(service.plan(desired, ["public"])).rejects.toMatchObject({
      code: "VALIDATION_ERROR",
      message: expect.stringContaining("cannot declare an identity"),
    });
    expect(
      (await client.query(`
        SELECT
          to_regclass('public.unrelated_generated_identity_records') AS unrelated,
          to_regclass('public.generated_identity_records') AS invalid
      `)).rows
    ).toEqual([{ unrelated: null, invalid: null }]);
  });
  ...
  test("rejects a virtual generated expression that invokes an unqualified user-defined operator", async function () {
    await client.query(`
      CREATE FUNCTION public.existing_virtual_unqualified_operator(left_value text, right_value text)
      RETURNS boolean
      LANGUAGE sql
      IMMUTABLE
      AS $$ SELECT left_value = right_value; $$
    ...
    await client.query(`
      CREATE OPERATOR public.#=# (
        PROCEDURE = public.existing_virtual_unqualified_operator,
        LEFTARG = text,
        RIGHTARG = text
      )
    `);
    ...
    const desired = `
      ...
      CREATE TABLE public.unrelated_virtual_unqualified_operator_records (
        id integer PRIMARY KEY
      );

      CREATE TABLE public.existing_virtual_unqualified_operator_records (
        source text NOT NULL,
        matches_source boolean GENERATED ALWAYS AS (source #=# 'expected') VIRTUAL
      );
    `;
    ...
    expect(
      (await client.query(
        "SELECT to_regclass('public.unrelated_virtual_unqualified_operator_records') AS relation"
      )).rows
    ).toEqual([{ relation: null }]);
  });
});

...
describe("Parser gap coverage", function () {
  test("rejects conflicting serial clauses without broadening ordinary columns", async function () {
    ...
    for (const definition of invalidDefinitions) {
      await expect(
        ...
        message: expect.stringContaining(
          "conflicts with SERIAL's implicit NOT NULL and sequence-backed default"
        ),
      });
    }

    const parsed = await parser.parseSchema(`
      CREATE TABLE valid_serial (
        id SERIAL NOT NULL,
        ordinary INTEGER DEFAULT 42
      );
    `);

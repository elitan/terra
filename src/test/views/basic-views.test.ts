...
describe("Basic View Operations", () => {
  ...
  describe("Complex Views", () => {
    test("should handle views with window functions", async () => {
      const schema = `
        CREATE TABLE sales (
          id SERIAL PRIMARY KEY,
      ...
      expect(result.rows[0].overall_rank).toBe('1'); // Top sale
    });

    test("should support views with CTEs (Common Table Expressions)", async () => {
      const schema = `

...
| table `MAINTAIN` privilege | 14-16 vs 17-18 | preserved unmanaged | PostgreSQL 17 adds the table `MAINTAIN` privilege. TerraDB's portable declarative object-grant and default-privilege contracts intentionally exclude it so one desired schema behaves identically on 14-18; existing `MAINTAIN` ACL entries are ignored and preserved rather than revoked. |
| large-object default privileges | 14-17 vs 18 | rejected | PostgreSQL 18 adds `ALTER DEFAULT PRIVILEGES ... ON LARGE OBJECTS`. TerraDB rejects that desired syntax and preserves existing `pg_default_acl` large-object rows unmanaged so the same desired schema remains portable across 14-18. |

...
| table `MAINTAIN` privilege | 14-17 vs 18 | preserved unmanaged | PostgreSQL 18 adds the table `MAINTAIN` privilege. TerraDB's portable declarative grant contract intentionally excludes it so one desired schema behaves identically on 14-18; existing `MAINTAIN` ACL entries are ignored and preserved rather than revoked. |

...
- done: reject same-apply user-defined functions in PostgreSQL virtual generated expressions during planning, preserving an empty target because PostgreSQL virtual expressions permit built-in functions/types only.
- done: reject same-apply PostgreSQL enum, composite, domain, and range types declared on virtual generated columns during planning, preserving an empty target instead of allowing PostgreSQL 18 to fail after earlier statements have run.

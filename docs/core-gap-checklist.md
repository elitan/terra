...
- done: classify emitted PostgreSQL role, grant, default-privilege, policy/RLS, and foreign-server statements in ordered CLI metadata instead of degrading them to `other`, with safe/destructive risk regression coverage.
- done: serialize the shared-database PostgreSQL property gate so unrelated property cases cannot clean or recreate `public` objects during another case's apply/re-plan assertion.
- done: give the changed-file mutation classifier preflight the same explicit per-test timeout as its composite-type regression suite, preventing a healthy slow CI case from being reported as a mutation failure.
- done: add PostgreSQL ordinary-index rollback integration coverage: an invalid later routine rolls back index creation, preserves table OID/data, leaves no index artifact, and replans the original schema empty.
- done: add PostgreSQL ordinary-constraint rollback integration coverage: an invalid later routine rolls back a prior check-constraint addition, preserves table OID/data, leaves no constraint artifact, and replans the original schema empty.
- done: create same-apply PostgreSQL functions before new tables that reference them from stored generated expressions, and normalize a local schema qualifier in those expressions for an empty re-plan without masking cross-schema calls.
- done: reject desired or existing user-defined functions in PostgreSQL virtual generated expressions during planning, preserving an empty target because PostgreSQL virtual expressions permit built-in functions/types only.
- done: reject desired or existing PostgreSQL enum, composite (including table row), domain, and range types declared on or referenced by virtual generated columns—including expression casts and referenced user-typed columns—during planning, preserving an empty target instead of allowing PostgreSQL 18 to fail after earlier statements have run.
- done: reject user-defined operators in PostgreSQL virtual generated expressions during planning, including explicitly schema-qualified operators and unqualified operators whose name has no built-in counterpart; permit explicitly qualified `pg_catalog` operators and normal built-in operator names.
...
- done: reject PostgreSQL inheritance declarations that change an inherited column into or out of generated form, use a different generated storage kind, or merge incompatible generated parent columns before surrounding DDL can mutate on 14-18.
...
- done: preserve PostgreSQL DEFAULT/FULL/NOTHING/USING INDEX replica identity on ordinary and partition relations; inspect missing selected indexes, validate index eligibility before mutation, reset and restore identity around dependency changes, keep constraint-backed work atomic, and execute concurrent standalone index builds before dependent assignments on PostgreSQL 14-18; a later duplicate-data constraint failure rolls an earlier identity change back without changing relation OID or rows.
...
- done: make PostgreSQL concurrent-index work lossless: reject mixed or multiple explicit concurrent statements before mutation, execute new-table `CONCURRENTLY` declarations plus dependent assignments transactionally, render generated removals transactionally whenever they share other work, retain an otherwise singleton removal as concurrent, and remove only a newly-created invalid index artifact when its one standalone concurrent creation fails.

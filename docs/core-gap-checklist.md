...
- done: reject desired or existing PostgreSQL enum, composite (including table row), domain, and range types declared on or referenced by virtual generated columns during planning, preserving an empty target instead of allowing PostgreSQL 18 to fail after earlier statements have run.

...
| views | covered | covered | PostgreSQL materialized-view indexes preserve complete metadata through creation, inspection, external convergence, targeted replacement/removal, view recreation, schema isolation, and idempotent reapply; materialized-view `WITH [NO] DATA` population state is parsed, inspected, transitioned transactionally, and reapplied idempotently; ordinary-view index targets fail before planning; SQLite full definitions, explicit column lists, replacement, recreation ordering, and rollback covered |
